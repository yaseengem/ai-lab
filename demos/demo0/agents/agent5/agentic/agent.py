...
from .knowledge import search_trianz_knowledge  # noqa: E402
from .sales_agent import recommend_offering, capture_lead  # noqa: E402
from .scheduling_agent import request_human_meeting  # noqa: E402
...
def create_agent(
    ...
    `user_id` carries the verified visitor email so tools can pre-fill it.
    """
    ...
    except Exception:  # pragma: no cover - knowledge must never break chat
        overview = ""

    context_block = (
        f"\n\n=== TRIANZ OVERVIEW (for grounding; use search_trianz_knowledge for detail) ===\n"
        f"{overview}\n"
        f"\n=== CURRENT SESSION CONTEXT ===\n"
        f"Persona:        {persona}\n"
        f"Visitor email:  {user_id or 'unknown'}\n"
