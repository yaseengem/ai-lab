...
Run from repo root:
  uvicorn agents.agent5.apis.main:app --host 0.0.0.0 --port 3098 --reload
"""
...
from agents.agent5.agentic.paths import AGENT_DIR as _AGENT_DIR, LOGS_DIR as _LOGS_DIR, ensure_state_dirs
from agents.agent5.agentic import knowledge as _knowledge
...
# Build the Trianz knowledge index from content/ (rebuildable; safe if content/ is empty).
try:
    _idx = _knowledge.ensure_index()
    logging.getLogger(__name__).info(
        "[STARTUP] knowledge_index docs=%s files=%s",
        _idx.get("doc_count"), _idx.get("file_count"))
except Exception as _exc:  # pragma: no cover - knowledge must never block boot
    logging.getLogger(__name__).warning("[STARTUP] knowledge_index_failed error=%s", _exc)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("agents.agent5.apis.main:app", host="0.0.0.0",
