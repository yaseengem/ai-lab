...
from agents.agent5.agentic.approval_hook import ApprovalHook, APPROVE  # noqa: E402
from agents.agent5.agentic.memory_backend import get_memory_store  # noqa: E402
from agents.agent5.agentic.model import resolve_model_id  # noqa: E402
from agents.agent5.agentic.paths import (  # noqa: E402
...
class Service:
    ...
    def self_check() -> dict:
        ...
        # 4) Trianz knowledge index (informative — empty content/ is allowed).
        try:
            from agents.agent5.agentic import knowledge
            idx = knowledge.ensure_index()
            checks.append({"name": "knowledge", "ok": True,
                           "detail": f"{idx.get('doc_count', 0)} chunks from {idx.get('file_count', 0)} file(s)"})
        except Exception as e:  # pragma: no cover
            checks.append({"name": "knowledge", "ok": True, "detail": f"index unavailable: {e}"})

        # 5) SES sender (informative — OTP falls back to dev delivery when unset).
        sender = str((cfg.get("defaults") or {}).get("ses_sender") or "").strip()
        checks.append({"name": "ses_sender", "ok": True,
                       "detail": f"sender={sender}" if sender else "no ses_sender — OTP/invites run in dev mode"})

        # 6) Nova Sonic voice backend (informative — text path works regardless).
        try:
            from agents.agent5.agentic import sonic_session
            voice_ok, voice_detail = sonic_session.available()
        except Exception as e:  # pragma: no cover
            voice_ok, voice_detail = False, str(e)
        checks.append({"name": "voice_sonic", "ok": True,
                       "detail": voice_detail if voice_ok else f"{voice_detail} (text chat still available)"})
